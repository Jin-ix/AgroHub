...
<div class="container">
    ...
    if (!isset($_SESSION['BuyerID'])) {
        echo "<div class='alert alert-danger'>You are not logged in. Please log in to view your order history.</div>";
        exit();
    }

    $BuyerID = $_SESSION['BuyerID'];
    $fname = isset($_SESSION['fname']) ? htmlspecialchars($_SESSION['fname']) : 'User';

    echo "<h2>Welcome, $fname</h2>";

    $sql = "SELECT oh.*, r.star, r.review, r.image AS reviewImage
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($isCancelled) {
                ...
                echo '<td>
                      <button class="btn btn-info"><a href="complaint.php?orderID=' . $orderID .  '&SellerName=' . urlencode($SellerName) . '" style="color: white; text-decoration: none;">Help</a></button>
                      </td>';
            }
